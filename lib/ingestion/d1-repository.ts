...
export class D1IngestionRepository implements IngestionRepository {
  ...
  async saveAdvisory(runId: string, advisory: NormalizedAdvisory, changeTypes: string[]): Promise<"inserted" | "changed" | "unchanged"> {
    ...
    if (advisory.releaseEvent) {
      queries.push(this.db.prepare("INSERT INTO release_events (id, vendor_id, event_type, event_date, label, source_url, reported_cve_count, reported_at, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(id) DO UPDATE SET label=excluded.label, source_url=excluded.source_url, reported_cve_count=COALESCE(excluded.reported_cve_count, release_events.reported_cve_count), reported_at=COALESCE(excluded.reported_at, release_events.reported_at), updated_at=excluded.updated_at").bind(advisory.releaseEvent.id, advisory.vendor, advisory.releaseEvent.eventType, advisory.releaseEvent.eventDate, advisory.releaseEvent.label, advisory.releaseEvent.sourceUrl ?? advisory.sourceUrl, advisory.releaseEvent.reportedCveCount ?? null, advisory.releaseEvent.reportedAt ?? null, now, now));
