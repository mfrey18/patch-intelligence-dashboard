...
export async function refreshDashboardProjection(
  ...
  try {
    ...
    await db.prepare("SELECT pg_advisory_xact_lock(783401)").run();
    ...
    await db.batch([
      ...
      db.prepare(`INSERT INTO cve_dashboard_facts (${FACT_COLUMNS}) SELECT ${FACT_COLUMNS} FROM cve_dashboard_facts_staging ON CONFLICT DO NOTHING`),
...
function staticFactsUpdateSql(): string {
  ...
  const changed = STATIC_FACT_COLUMNS
    .map((column) => `cve_dashboard_facts.${column} IS DISTINCT FROM s.${column}`).join(" OR ");
  ...
  return `UPDATE cve_dashboard_facts SET
    ...
    WHERE EXISTS(SELECT 1 FROM cve_dashboard_facts_staging s
      WHERE s.cve_id=cve_dashboard_facts.cve_id AND (
        cve_dashboard_facts.epss IS DISTINCT FROM s.epss OR
        cve_dashboard_facts.epss_percentile IS DISTINCT FROM s.epss_percentile OR
        cve_dashboard_facts.priority IS DISTINCT FROM s.priority
      ))`;
}
...
WITH current_epss AS (
  ...
  WHERE ed.is_current=TRUE AND ed.status='published'
), latest_revisions AS (
...
SELECT cve_id,title,vendor,vendor_ids,product,severity_rank,cvss,epss,epss_percentile,
  ...
  CASE WHEN kev=TRUE OR known_exploited=TRUE THEN 'P1'
...
async function acquireProjectionLease(db: Database, holder: string, now: Date): Promise<boolean> {
...
async function queryStoredProjectionParity(db: Database, table: "cve_dashboard_facts_staging"): Promise<ProjectionParityMetrics> {
  const row = await db.prepare(`SELECT COUNT(*) total,COALESCE(SUM((severity_rank=4)::integer),0) critical,COALESCE(SUM((severity_rank=3)::integer),0) high,
    COALESCE(SUM((known_exploited)::integer),0) known_exploited,COALESCE(SUM((kev)::integer),0) kev,COALESCE(SUM((zero_day)::integer),0) zero_day,
    COALESCE(SUM((patch_available=TRUE)::integer),0) patch_available,COALESCE(SUM((priority='P1')::integer),0) p1,COALESCE(SUM((priority='P2')::integer),0) p2,
    COALESCE(SUM((strpos(vendor_ids,'|microsoft|')>0)::integer),0) microsoft,COALESCE(SUM((strpos(vendor_ids,'|cisco|')>0)::integer),0) cisco FROM ${table}`).first<Record<string, number>>();
