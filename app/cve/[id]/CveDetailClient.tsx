...
export function CveDetailClient({ cveId, apiBaseUrl = "", backHref = "/" }: { cveId: string; apiBaseUrl?: string; backHref?: string }) {
  ...
  return <main className="detailShell">
    ...
    <section className="detailGrid">
      ...
      <article className="panel detailPanel"><SectionTitle eyebrow="FIRST predictive enrichment" title="EPSS history" />{detail.epss.history.length ? <><EpssGraph history={detail.epss.history} /><small>FIRST EPSS · current dataset observed {dateTime(detail.epss.history.at(-1)!.observedAt)} · predictive enrichment, not exploitation evidence</small></> : <Empty message="No EPSS observations are available for this CVE." />}</article>
    </section>

    <section className="detailGrid">
      <article className="panel detailPanel"><SectionTitle eyebrow="Evidence with provenance" title="Exploitation evidence" />{detail.exploitation.evidence.length ? <div className="evidenceList">{detail.exploitation.evidence.map((item, index) => <article key={`${item.type}-${item.url}-${index}`}><i /><div><strong>{titleCase(item.type)} · {titleCase(item.status)}</strong><small>{item.source} ({item.sourceId}){item.date ? ` · evidence ${date(item.date)}` : ""} · observed {dateTime(item.observedAt)}</small><p>{item.summary ?? "Authoritative source assertion."}</p><a href={item.url} target="_blank" rel="noreferrer">Evidence source ↗</a></div></article>)}</div> : <Empty message="No authoritative exploitation evidence is present." />}</article>
