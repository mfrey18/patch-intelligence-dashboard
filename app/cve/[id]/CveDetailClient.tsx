...
export function CveDetailClient({ cveId, apiBaseUrl = "", backHref = "/" }: { cveId: string; apiBaseUrl?: string; backHref?: string }) {
  ...
  return <main className="detailShell">
    ...
    <section className="panel detailPanel fullPanel"><SectionTitle eyebrow="Supporting vendor context" title="Vendor Remediation Information" />{detail.remediations.length ? <div className="remediationList">{detail.remediations.map((item, index) => <article key={`${item.vendor}-${item.kind}-${index}`}><span>{titleCase(item.kind)}</span><strong>{item.vendor}{item.fixedVersion ? ` · Fixed in ${item.fixedVersion}` : ""}</strong><p>{item.action ?? "Refer to the authoritative source for action details."}</p>{item.products.length ? <small>{productScope(item.products)}</small> : null}<small>{item.patchAvailable === true ? "Patch explicitly available" : item.patchAvailable === false ? "Patch explicitly unavailable" : "Patch availability not stated"}{item.rebootRequired === true ? " · Reboot required" : ""} · {item.sourceId} · observed {dateTime(item.observedAt)}</small><a href={item.sourceUrl} target="_blank" rel="noreferrer">Source ↗</a></article>)}</div> : <Empty message="No structured vendor remediation information is currently available." />}</section>
  </main>;
}
...
function productScope(products: string[]) { const examples = products.slice(0, 3).join(" · "); return `Applies to ${products.length} product/version record${products.length === 1 ? "" : "s"}: ${examples}${products.length > 3 ? ` · +${products.length - 3} more` : ""}`; }
