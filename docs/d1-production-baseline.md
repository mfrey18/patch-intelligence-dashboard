...
Migration `0005_dashboard_read_model.sql` adds a disposable `cve_dashboard_facts` projection and targeted revision-first/release-event indexes. Canonical CVE, advisory, product, remediation, exploitation, KEV, and EPSS tables remain authoritative. Projection publication is atomic, so a failed refresh preserves the last-known-good facts and state. The Worker retains the canonical relational query as an expand/deploy/populate fallback until a non-empty projection is published.
