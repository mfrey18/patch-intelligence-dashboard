...
export function PatchTuesdayArchiveClient({ apiBaseUrl, backHref = "/" }: { apiBaseUrl: string; backHref?: string }) {
  ...
  return <main className="detailShell operationalShell">
    ...
    <section className="detailHero"><div><p className="eyebrow">Release-event intelligence</p><h1>Patch Tuesday archive</h1><p>Microsoft’s reported total and CVEs linked to advisory assertions are shown separately. Severity, exploitation, KEV, zero-day, and product metrics use linked CVEs only.</p></div></section>
    ...
    <section className="releaseArchive">{events.map((event) => <article className="panel" key={event.id}><div className="panelHead"><div><p className="eyebrow">{event.eventDate} · {label(event.reconciliationStatus)}</p><h2>{event.label}</h2></div>{event.totalSourceUrl && <a href={event.totalSourceUrl} target="_blank" rel="noreferrer">Microsoft source ↗</a>}</div><div className="releaseReconciliation"><Stat label="Microsoft reported" value={event.totalBasis === "vendor_reported" ? event.total : null} /><Stat label="Linked CVEs" value={event.linkedTotal} /><Stat label="Coverage" value={event.linkCoveragePercent == null ? null : `${event.linkCoveragePercent}%`} /><Stat label="Critical" value={event.critical} /><Stat label="High" value={event.high} /><Stat label="Known exploited" value={event.knownExploited} /><Stat label="Zero-days" value={event.zeroDay} /><Stat label="CISA KEV" value={event.kev} /></div>{event.comparison && <p className="eventProvenance">Versus {event.comparison.label}: reported {signed(event.comparison.totalDelta)} · linked {signed(event.comparison.linkedTotalDelta)} · Critical {signed(event.comparison.criticalDelta)} · High {signed(event.comparison.highDelta)} · exploited {signed(event.comparison.knownExploitedDelta)}.</p>}<p className="panelNote">{event.productFamilyBasis === "vendor_reported" ? "Microsoft-reported affected CVEs by family; counts overlap when CVEs affect multiple families." : "Product families calculated from linked advisory assertions."}</p><div className="archiveProducts">{event.productFamilies.map((product) => <span key={product.label}><strong>{product.value}</strong>{product.label}</span>)}</div></article>)}</section>
  </main>;
}
...
function label(value: string) { return value === "matched" ? "Reported and linked totals match" : value === "partial" ? "Some reported CVEs are not linked" : value === "overlinked" ? "Linked count exceeds reported total" : "Microsoft total not captured"; }
