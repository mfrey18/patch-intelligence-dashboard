import { scopePredicate, severitySql, intelligenceColumns } from "./intelligence-sql";
...
interface BaseRow {
  vulncheck?: boolean; exploitation_sources?: string | string[]; assessment_source?: string | null; cve_id: string; title: string; vendor: string; product: string | null; severity_rank: number; cvss: number | null; epss: number | null; epss_percentile: number | null; kev: boolean; known_exploited: boolean; zero_day: boolean; patch_available: boolean | null; mitigation_available: boolean; workaround_available: boolean; published_at: string | null; modified_at: string | null;
}

export async function queryDashboard(db: Database, url: URL): Promise<DashboardResponse> {
  ...
  const [
    ...
    db.prepare(`${cte} SELECT *,${intelligenceColumns("filtered.cve_id")} FROM filtered ORDER BY ${sort} LIMIT ? OFFSET ?`).bind(...bindings, limit + 1, offset).all<BaseRow>(),
    db.prepare(`${cte} SELECT COUNT(*) total, COALESCE(SUM((severity_rank=4)::integer),0) critical, COALESCE(SUM((severity_rank=3)::integer),0) high, COALESCE(SUM((known_exploited)::integer),0) known_exploited, COALESCE(SUM((kev)::integer),0) kev, COALESCE(SUM((zero_day)::integer),0) zero_day, COALESCE(SUM((patch_available=TRUE)::integer),0) patch_available, COALESCE(SUM((kev=TRUE OR known_exploited=TRUE)::integer),0) p1, COALESCE(SUM((kev=FALSE AND known_exploited=FALSE AND ((severity_rank=4 AND epss_percentile>=?) OR (severity_rank=3 AND epss_percentile>=?)))::integer),0) p2 FROM filtered`).bind(...bindings, PRIORITY_THRESHOLDS.criticalHighEpssPercentile, PRIORITY_THRESHOLDS.highVeryHighEpssPercentile).first<Record<string, number>>(),
...
export async function queryDashboardAnalytics(db: Database, url: URL, panel: DashboardAnalyticsPanel): Promise<DashboardAnalyticsResponse> {
  const startedAt = performance.now();
...
export async function queryDashboardExport(db: Database, url: URL): Promise<{ generatedAt: string; rows: DashboardVulnerabilityRow[]; nextCursor: string | null }> {
  const projection = await hasPublishedProjection(db);
  const { cte, bindings } = projection ? buildProjectedFilteredCte(url.searchParams) : buildCanonicalFilteredCte(url.searchParams);
  const limit = clamp(Number(url.searchParams.get("limit") ?? 1_000), 1, 1_000);
  const offset = decodeCursor(url.searchParams.get("cursor"));
  const result = await db.prepare(`${cte} SELECT *,${intelligenceColumns("filtered.cve_id")} FROM filtered ORDER BY ${sortSql(url.searchParams.get("sort"))} LIMIT ? OFFSET ?`).bind(...bindings, limit + 1, offset).all<BaseRow>();
...
async function queryRecentChanges(db: Database, cte: string, bindings: unknown[]): Promise<DashboardResponse["recentChanges"]> {
...
async function queryActivity(db: Database, cte: string, bindings: unknown[]): Promise<Pick<DashboardResponse, "vulnerabilityActivity" | "threatSignalActivity">> {
  const result = await db.prepare(`${cte} SELECT to_char(published_at,'YYYY-MM') bucket,
    COALESCE(SUM((severity_rank=4)::integer),0) critical, COALESCE(SUM((severity_rank=3)::integer),0) high, COALESCE(SUM((severity_rank=2)::integer),0) medium, COALESCE(SUM((severity_rank=1)::integer),0) low,
    COALESCE(SUM((known_exploited)::integer),0) known_exploited, COALESCE(SUM((kev)::integer),0) kev, COALESCE(SUM((zero_day)::integer),0) zero_day, COALESCE(SUM((epss_percentile>=?)::integer),0) high_epss
    FROM filtered WHERE published_at IS NOT NULL GROUP BY to_char(published_at,'YYYY-MM') ORDER BY bucket`).bind(...bindings, HIGH_EPSS_PERCENTILE).all<Record<string, unknown>>();
...
async function hasPublishedProjection(db: Database): Promise<boolean> {
  try {
    const state = await db.prepare("SELECT cve_count FROM dashboard_projection_state WHERE id='current' AND status='published' AND projection_version=2").first<{ cve_count: number }>();
...
export async function queryCanonicalProjectionParity(db: Database): Promise<ProjectionParityMetrics> {
  const row = await db.prepare(`${canonicalParityCte()} SELECT COUNT(*) total,
    COALESCE(SUM((severity_rank=4)::integer),0) critical,COALESCE(SUM((severity_rank=3)::integer),0) high,
    COALESCE(SUM((known_exploited)::integer),0) known_exploited,COALESCE(SUM((kev)::integer),0) kev,
    COALESCE(SUM((zero_day)::integer),0) zero_day,COALESCE(SUM((patch_available=TRUE)::integer),0) patch_available,
    COALESCE(SUM((kev=TRUE OR known_exploited=TRUE)::integer),0) p1,
    COALESCE(SUM((kev=FALSE AND known_exploited=FALSE AND ((severity_rank=4 AND epss_percentile>=?) OR (severity_rank=3 AND epss_percentile>=?)))::integer),0) p2,
    COALESCE(SUM((microsoft)::integer),0) microsoft,COALESCE(SUM((cisco)::integer),0) cisco
...
function canonicalParityCte(): string { return `WITH current_epss AS (
  ...
  JOIN epss_datasets ed ON ed.score_date=eo.score_date AND ed.is_current=TRUE AND ed.status='published'
), latest_revisions AS (
  ...
  SELECT cve_id,BOOL_OR(patch_available) patch_available
  ...
  SELECT c.id cve_id,
    ...
    BOOL_OR(rf.patch_available) patch_available,
    BOOL_OR(a.vendor_id='microsoft') microsoft,BOOL_OR(a.vendor_id='cisco') cisco
...
async function queryEpssMovers(db: Database, cte: string, bindings: unknown[]): Promise<DashboardResponse["epssMovers"]> {
  const result = await db.prepare(`${cte}, current_points AS (
      SELECT eo.cve_id, eo.score_date, eo.score, eo.percentile, eo.model_version FROM epss_observations eo JOIN epss_datasets ed ON ed.score_date=eo.score_date AND ed.is_current=TRUE AND ed.status='published'
    ), prior_dates AS (
      SELECT cp.cve_id, (SELECT po.score_date FROM epss_observations po JOIN epss_datasets pd ON pd.score_date=po.score_date AND pd.status='published' WHERE po.cve_id=cp.cve_id AND po.score_date BETWEEN (cp.score_date + ?::interval) AND (cp.score_date + ?::interval) AND COALESCE(po.model_version,'')=COALESCE(cp.model_version,'') ORDER BY po.score_date DESC LIMIT 1) previous_score_date FROM current_points cp
    )
...
async function queryEmergingVulnerabilities(db: Database, cte: string, bindings: unknown[]): Promise<DashboardResponse["emergingVulnerabilities"]> {
  const recentSince = `-${EMERGING_CHANGE_WINDOW_DAYS} days`;
  const result = await db.prepare(`${cte} SELECT f.*, changes.recent_change_types, changes.latest_change_at
    FROM filtered f LEFT JOIN LATERAL (SELECT STRING_AGG(DISTINCT ic.change_type, ',' ORDER BY ic.change_type) recent_change_types, MAX(ic.observed_at) latest_change_at FROM intelligence_changes ic WHERE ic.cve_id=f.cve_id AND ic.observed_at>=(CURRENT_TIMESTAMP + ?::interval)) changes ON TRUE
    WHERE EXISTS(SELECT 1 FROM intelligence_changes ec WHERE ec.cve_id=f.cve_id AND ec.observed_at>=(CURRENT_TIMESTAMP + ?::interval) AND ec.change_type IN ('EXPLOITATION_STATUS_CHANGED','ZERO_DAY_STATUS_CHANGED','KEV_ADDED','SEVERITY_CHANGED','CVSS_CHANGED'))
      OR (f.severity_rank=4 AND f.epss_percentile>=?) OR (f.severity_rank=3 AND f.epss_percentile>=?)
    ORDER BY f.known_exploited DESC, f.kev DESC, f.zero_day DESC, latest_change_at DESC, f.epss_percentile DESC NULLS LAST LIMIT 12`).bind(...bindings, recentSince, recentSince, HIGH_EPSS_PERCENTILE, PRIORITY_THRESHOLDS.highVeryHighEpssPercentile).all<BaseRow & { recent_change_types: string | null; latest_change_at: string | null }>();
  return (result.results ?? []).map((row) => {
...
async function queryVendorThreatSeries(db: Database, cte: string, bindings: unknown[]): Promise<DashboardResponse["vendorThreatSeries"]> {
  const result = await db.prepare(`${cte} SELECT f.vendor label, COUNT(*) total,
    COALESCE(SUM((f.known_exploited)::integer),0) known_exploited,
    COALESCE(SUM((f.kev)::integer),0) kev,
    COALESCE(SUM((f.zero_day)::integer),0) zero_day,
    COALESCE(SUM((f.epss_percentile>=?)::integer),0) high_epss
...
async function queryChangeCategoryCounts(db: Database, cte: string, bindings: unknown[]): Promise<DashboardResponse["changeCategoryCounts"]> {
  const result = await db.prepare(`${cte} SELECT ic.change_type, COUNT(DISTINCT ic.id) value FROM intelligence_changes ic JOIN filtered f ON f.cve_id=ic.cve_id WHERE ic.observed_at>=(CURRENT_TIMESTAMP + INTERVAL '-30 days') GROUP BY ic.change_type`).bind(...bindings).all<{ change_type: string; value: number }>();
...
async function queryCweAnalytics(db: Database, cte: string, bindings: unknown[]): Promise<DashboardResponse["cweAnalytics"]> {
  const [coverage, series] = await Promise.all([
    db.prepare(`${cte} SELECT COUNT(*) total, COALESCE(SUM((cwe IS NOT NULL AND trim(cwe)<>'')::integer),0) known FROM filtered`).bind(...bindings).first<Record<string, number>>(),
    db.prepare(`${cte} SELECT cwe label, COUNT(*) value, COALESCE(SUM((severity_rank=4)::integer),0) critical, COALESCE(SUM((known_exploited)::integer),0) exploited FROM filtered WHERE cwe IS NOT NULL AND trim(cwe)<>'' GROUP BY cwe ORDER BY value DESC LIMIT 10`).bind(...bindings).all<Record<string, unknown>>(),
  ]);
...
function buildProjectedFilteredCte(params: URLSearchParams): { cte: string; bindings: unknown[] } {
  ...
  if (params.get("vendor")) add("strpos(vendor_ids, '|' || ? || '|') > 0", params.get("vendor"));
  if (params.get("product")) add("product ILIKE ? ESCAPE '\\'", `%${escapeLike(params.get("product")!)}%`);
  ...
  if (params.get("q")) {
    ...
    add("(cve_id ILIKE ? ESCAPE '\\' OR title ILIKE ? ESCAPE '\\' OR vendor ILIKE ? ESCAPE '\\' OR product ILIKE ? ESCAPE '\\')", term, term, term, term);
  }
  if (params.get("publishedFrom")) add("published_at >= ?", params.get("publishedFrom"));
  if (params.get("publishedTo")) add("published_at < (?::date + INTERVAL '1 day')", params.get("publishedTo"));
  if (params.get("modifiedFrom")) add("modified_at >= ?", params.get("modifiedFrom"));
  if (params.get("modifiedTo")) add("modified_at < (?::date + INTERVAL '1 day')", params.get("modifiedTo"));
  ...
  booleanFilterOuter(params, add, "vulncheck", "EXISTS(SELECT 1 FROM vulncheck_entries vf WHERE vf.cve_id=cve_dashboard_facts.cve_id AND vf.active=TRUE)");
  if (params.get("exploitationSource")) add("EXISTS(SELECT 1 FROM exploit_evidence sf WHERE sf.cve_id=cve_dashboard_facts.cve_id AND sf.source_id=? AND sf.evidence_type='known_exploitation' AND sf.status='confirmed')",params.get("exploitationSource"));
  ...
  if (view === "changed") add("EXISTS(SELECT 1 FROM intelligence_changes view_changes WHERE view_changes.cve_id=cve_dashboard_facts.cve_id AND view_changes.observed_at>=(CURRENT_TIMESTAMP + INTERVAL '-1 day'))");
  if (view === "patch-new") add("patch_available=TRUE AND EXISTS(SELECT 1 FROM intelligence_changes view_changes WHERE view_changes.cve_id=cve_dashboard_facts.cve_id AND view_changes.observed_at>=(CURRENT_TIMESTAMP + INTERVAL '-1 day') AND view_changes.change_type IN ('REMEDIATION_CHANGED','FIXED_VERSION_CHANGED','MITIGATION_ADDED','WORKAROUND_ADDED'))");
...
function buildCanonicalFilteredCte(params: URLSearchParams): { cte: string; bindings: unknown[] } {
  ...
  if (params.get("product")) add("EXISTS(SELECT 1 FROM affected_products apf JOIN products pf ON pf.id=apf.product_id WHERE apf.advisory_id=a.id AND apf.advisory_revision_id=ar.id AND (apf.cve_id=c.id OR apf.cve_id IS NULL) AND pf.name ILIKE ?)", `%${escapeLike(params.get("product")!)}%`);
  const booleanFilter = (key: string, expression: string) => { const value = params.get(key); if (value === "true" || value === "false") add(`${expression} = ?`, value === "true"); };
  booleanFilter("vulncheck", "EXISTS(SELECT 1 FROM vulncheck_entries vf WHERE vf.cve_id=c.id AND vf.active=TRUE)");
  if (params.get("exploitationSource")) { where.push("EXISTS(SELECT 1 FROM exploit_evidence sf WHERE sf.cve_id=c.id AND sf.source_id=? AND sf.evidence_type='known_exploitation' AND sf.status='confirmed')"); bindings.push(params.get("exploitationSource")); }
  booleanFilter("kev", "EXISTS(SELECT 1 FROM kev_entries kf WHERE kf.cve_id=c.id AND kf.active=TRUE)");
  ...
  if (params.get("q")) {
    const term = `%${escapeLike(params.get("q")!.slice(0, 100))}%`;
    addOuter("(cve_id ILIKE ? OR title ILIKE ? OR vendor ILIKE ? OR product ILIKE ?)", term, term, term, term);
  }
  if (params.get("publishedFrom")) addOuter("published_at >= ?", params.get("publishedFrom"));
  if (params.get("publishedTo")) addOuter("published_at < (?::date + INTERVAL '1 day')", params.get("publishedTo"));
  if (params.get("modifiedFrom")) addOuter("modified_at >= ?", params.get("modifiedFrom"));
  if (params.get("modifiedTo")) addOuter("modified_at < (?::date + INTERVAL '1 day')", params.get("modifiedTo"));
  if(params.get("severity")) addOuter("severity_rank=?", ({critical:4,high:3,medium:2,low:1} as Record<string,number>)[params.get("severity")!.toLowerCase()] ?? 0);
  ...
  const priority = params.get("priority")?.toUpperCase();
  if (priority === "P1") outer.push("(kev=TRUE OR known_exploited=TRUE)");
  if (priority === "P2") { outer.push("kev=FALSE AND known_exploited=FALSE AND ((severity_rank=4 AND epss_percentile>=?) OR (severity_rank=3 AND epss_percentile>=?))"); bindings.push(PRIORITY_THRESHOLDS.criticalHighEpssPercentile, PRIORITY_THRESHOLDS.highVeryHighEpssPercentile); }
  if (priority === "P3") { outer.push("kev=FALSE AND known_exploited=FALSE AND NOT ((severity_rank=4 AND COALESCE(epss_percentile,0)>=?) OR (severity_rank=3 AND COALESCE(epss_percentile,0)>=?))"); bindings.push(PRIORITY_THRESHOLDS.criticalHighEpssPercentile, PRIORITY_THRESHOLDS.highVeryHighEpssPercentile); }
  const view = params.get("view");
  if (view === "needs-action") outer.push("(kev=TRUE OR known_exploited=TRUE)");
  if (view === "changed") outer.push("EXISTS(SELECT 1 FROM intelligence_changes view_changes WHERE view_changes.cve_id=base.cve_id AND view_changes.observed_at>=(CURRENT_TIMESTAMP + INTERVAL '-1 day'))");
  if (view === "patch-new") outer.push("patch_available=TRUE AND EXISTS(SELECT 1 FROM intelligence_changes view_changes WHERE view_changes.cve_id=base.cve_id AND view_changes.observed_at>=(CURRENT_TIMESTAMP + INTERVAL '-1 day') AND view_changes.change_type IN ('REMEDIATION_CHANGED','FIXED_VERSION_CHANGED','MITIGATION_ADDED','WORKAROUND_ADDED'))");

  const cte = `WITH current_epss AS (
    SELECT eo.cve_id, eo.score, eo.percentile FROM epss_observations eo JOIN epss_datasets ed ON ed.score_date=eo.score_date AND ed.is_current=TRUE
  ), latest_revisions AS (
    SELECT ar.id, ar.advisory_id FROM advisory_revisions ar
    WHERE ar.id=(SELECT ar2.id FROM advisory_revisions ar2 WHERE ar2.advisory_id=ar.advisory_id ORDER BY ar2.observed_at DESC LIMIT 1)
    ...
    WHERE COALESCE(pa.published_at,pa.source_updated_at)>=(CURRENT_TIMESTAMP + INTERVAL '-${INTELLIGENCE_WINDOW_MONTHS} months')
    ...
    SELECT cve_id,STRING_AGG(DISTINCT name, ',' ORDER BY name) product FROM product_assertions GROUP BY cve_id
  ), current_remediation_rows AS (
    ...
    SELECT cve_id,
      BOOL_OR(patch_available) patch_available,
      BOOL_OR(kind='mitigation') mitigation_available,
      BOOL_OR(kind='workaround') workaround_available
    FROM remediation_assertions GROUP BY cve_id
  ), base AS (
    SELECT c.id cve_id, COALESCE(c.description, MAX(CASE WHEN a.id IS NOT NULL THEN ac.vendor_description END), MAX(a.title), c.id) title,
      ...
      EXISTS(SELECT 1 FROM exploit_evidence ee WHERE ee.cve_id=c.id AND ee.evidence_type='zero_day' AND ee.status='confirmed') zero_day,
      BOOL_OR(rf.patch_available) patch_available,
      COALESCE(BOOL_OR(rf.mitigation_available),FALSE) mitigation_available,
    ...
    WHERE ${scopePredicate()} AND ${where.join(" AND ")}
...
function booleanFilterOuter(params: URLSearchParams, add: (condition: string, ...values: unknown[]) => void, key: string, expression: string): void {
  ...
  if (value === "true" || value === "false") add(`${expression} = ?`, value === "true");
}

async function queryChanges(db: Database, cte: string, bindings: unknown[]): Promise<DashboardResponse["changes"]> {
  const prior = await db.prepare("SELECT MAX((SELECT r.started_at FROM source_runs r WHERE r.source_id=s.id AND r.completed_at IS NOT NULL ORDER BY r.completed_at DESC LIMIT 1 OFFSET 1)) started_at FROM sources s WHERE s.enabled=TRUE").first<{ started_at: string | null }>();
...
async function queryLatestReleaseEvent(db: Database): Promise<DashboardResponse["latestReleaseEvent"]> {
  return (await queryPatchTuesdayEvents(db, 2))[0] ?? null;
}

export async function queryPatchTuesdayEvents(db: Database, limit = 12): Promise<PatchTuesdayReleaseEvent[]> {
  ...
  const [statsResult, productsResult] = await Promise.all([
    db.prepare(`SELECT rea.release_event_id,
      ...
      COUNT(DISTINCT CASE WHEN EXISTS(SELECT 1 FROM kev_entries k WHERE k.cve_id=ac.cve_id AND k.active=TRUE) THEN ac.cve_id END) kev
...
function toDashboardRow(row: BaseRow): DashboardVulnerabilityRow {
  ...
  return { vulncheck: Boolean(row.vulncheck), exploitationSources: Array.isArray(row.exploitation_sources) ? row.exploitation_sources : JSON.parse(row.exploitation_sources ?? "[]"), assessmentSource: row.assessment_source ?? null, cveId: row.cve_id, title: row.title, vendor: row.vendor, product: row.product, severity, cvss: nullableNumber(row.cvss), epss: nullableNumber(row.epss), epssPercentile: nullableNumber(row.epss_percentile), kev, knownExploited, zeroDay: Boolean(row.zero_day), patchAvailable: row.patch_available == null ? null : Boolean(row.patch_available), mitigationAvailable: Boolean(row.mitigation_available), workaroundAvailable: Boolean(row.workaround_available), publishedAt: row.published_at, modifiedAt: row.modified_at, priority: calculatePriority({ kev, exploitationStatus: knownExploited ? "known_exploited" : "unknown", severity, cvss: nullableNumber(row.cvss), epssPercentile: nullableNumber(row.epss_percentile) }) };
}

function severityFromRank(rank: number): string { return rank === 4 ? "Critical" : rank === 3 ? "High" : rank === 2 ? "Medium" : rank === 1 ? "Low" : "Unknown"; }
function sortSql(value: string | null): string { return value === "epss" ? "epss_percentile DESC NULLS LAST, cve_id DESC" : value === "cvss" ? "cvss DESC NULLS LAST, cve_id DESC" : value === "modified" ? "modified_at DESC NULLS LAST, cve_id DESC" : value === "published" ? "published_at DESC NULLS LAST, cve_id DESC" : "kev DESC, known_exploited DESC, severity_rank DESC, epss_percentile DESC NULLS LAST, published_at DESC NULLS LAST, cve_id DESC"; }
