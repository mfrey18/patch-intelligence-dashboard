...
export class PostgresIngestionRepository implements IngestionRepository {
  ...
  async beginRun(sourceId: string, idempotencyKey: string | undefined, metadata: RunMetadata): Promise<{ runId: string; reused: boolean; continuation: string | null; boundHit: boolean }> {
    return this.db.transaction(async db => {
      await db.prepare("SELECT pg_advisory_xact_lock(hashtextextended(?,0))").bind(`run:${sourceId}`).run();
      return new PostgresIngestionRepository(db).beginRunLocked(sourceId, idempotencyKey, metadata);
    });
  }
  ...
  async saveAdvisory(runId: string, advisory: NormalizedAdvisory, changeTypes: string[]): Promise<"inserted" | "changed" | "unchanged"> {
    return this.db.transaction(async db => {
      await db.prepare("SELECT pg_advisory_xact_lock(hashtextextended(?,0))").bind(`advisory:${advisory.vendor}:${advisory.vendorAdvisoryId}`).run();
      return new PostgresIngestionRepository(db).saveAdvisoryLocked(runId, advisory, changeTypes);
    });
  }

  private async saveAdvisoryLocked(runId: string, advisory: NormalizedAdvisory, changeTypes: string[]): Promise<"inserted" | "changed" | "unchanged"> {
    ...
    if (advisory.releaseEvent) {
      queries.push(this.db.prepare("INSERT INTO release_events (id, vendor_id, event_type, event_date, label, source_url, reported_cve_count, reported_at, reported_product_families_json, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(id) DO UPDATE SET label=CASE WHEN excluded.reported_cve_count IS NOT NULL THEN excluded.label ELSE release_events.label END, source_url=CASE WHEN excluded.reported_cve_count IS NOT NULL THEN excluded.source_url ELSE COALESCE(release_events.source_url, excluded.source_url) END, reported_cve_count=COALESCE(excluded.reported_cve_count, release_events.reported_cve_count), reported_at=COALESCE(excluded.reported_at, release_events.reported_at), reported_product_families_json=COALESCE(excluded.reported_product_families_json, release_events.reported_product_families_json), updated_at=excluded.updated_at").bind(advisory.releaseEvent.id, advisory.vendor, advisory.releaseEvent.eventType, advisory.releaseEvent.eventDate, advisory.releaseEvent.label, advisory.releaseEvent.sourceUrl ?? advisory.sourceUrl, advisory.releaseEvent.reportedCveCount ?? null, advisory.releaseEvent.reportedAt ?? null, advisory.releaseEvent.reportedProductFamilies?.length ? JSON.stringify(advisory.releaseEvent.reportedProductFamilies) : null, now, now));
      queries.push(this.db.prepare("INSERT INTO release_event_advisories (release_event_id, advisory_id) VALUES (?, ?) ON CONFLICT DO NOTHING").bind(advisory.releaseEvent.id, advisoryId));
    }

    for (const assertion of advisory.cves) {
      queries.push(this.db.prepare("INSERT INTO cves (id, created_at, updated_at) VALUES (?, ?, ?) ON CONFLICT DO NOTHING").bind(assertion.cveId, now, now));
    ...
    // large bind lists become expensive. These JSON-backed INSERT...SELECT
    ...
    for (const remediation of advisory.remediations) {
      ...
      remediationRecords.push({ id: crypto.randomUUID(), cveId: remediation.cveId ?? null, productId, kind: remediation.kind, patchAvailable: remediation.patchAvailable ?? null, fixedVersion: remediation.fixedVersion ?? null, action: remediation.action ?? null, rebootRequired: remediation.rebootRequired ?? null, superseded: remediation.superseded ?? null, sourceUrl: remediation.sourceUrl, publishedAt: remediation.publishedAt ?? null, updatedAt: remediation.updatedAt ?? now });
    }
    ...
    if (productRecords.size > 0) queries.push(this.db.prepare(`INSERT INTO products (id, vendor_id, name, family, created_at, updated_at)
      SELECT (value->>'id'), ?, (value->>'name'), (value->>'family'), ?, ? FROM jsonb_array_elements(?::jsonb) AS items(value) WHERE TRUE
    ...
    if (affectedRecords.length > 0) queries.push(this.db.prepare(`INSERT INTO affected_products (id, advisory_id, advisory_revision_id, cve_id, product_id, affected_version, fixed_version, status, source_product_id)
      SELECT (value->>'id'), ?, ?, (value->>'cveId'), (value->>'productId'), (value->>'affectedVersion'), (value->>'fixedVersion'), (value->>'status'), (value->>'sourceProductId') FROM jsonb_array_elements(?::jsonb) AS items(value)`).bind(advisoryId, revisionId, JSON.stringify(affectedRecords)));
    if (remediationRecords.length > 0) queries.push(this.db.prepare(`INSERT INTO remediations (id, advisory_id, advisory_revision_id, cve_id, product_id, kind, patch_available, fixed_version, action, reboot_required, superseded, source_url, published_at, updated_at)
      SELECT (value->>'id'), ?, ?, (value->>'cveId'), (value->>'productId'), (value->>'kind'), (value->>'patchAvailable')::boolean, (value->>'fixedVersion'), (value->>'action'), (value->>'rebootRequired')::boolean, (value->>'superseded')::boolean, (value->>'sourceUrl'), (value->>'publishedAt')::timestamptz, (value->>'updatedAt')::timestamptz FROM jsonb_array_elements(?::jsonb) AS items(value)`).bind(advisoryId, revisionId, JSON.stringify(remediationRecords)));
